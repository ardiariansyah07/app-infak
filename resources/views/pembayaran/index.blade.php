...
<div class="card border-0 shadow-sm">
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light"><tr><th>Tanggal</th><th>Siswa</th><th>Nominal</th><th>Status</th><th>Bukti</th><th width="220">Aksi</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($pembayaran->status_verifikasi === 'pending')
                            <form action="{{ route($prefix . '.pembayaran.verify', $pembayaran) }}" method="POST" class="d-inline">
                                @csrf @method('PATCH')
                                <input type="hidden" name="status_verifikasi" value="valid">
                                <button class="btn btn-sm btn-success">Valid</button>
                            </form>
                            <form action="{{ route($prefix . '.pembayaran.verify', $pembayaran) }}" method="POST" class="d-inline">
                                @csrf @method('PATCH')
